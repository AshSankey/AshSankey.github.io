...
(function sankeymatic(glob) {
      ...
      newVals =
        panelEl.style.display === "none"
          ? {
              ...
              suffix: ":",
              action: String.fromCharCode(8211),
            }
  ...
  glob.updateOutput = (fld) => {
    ...
    switch (formats[fld]) {
      ...
      case ".2":
        oEl.textContent = d3.format(".2f")(fldValAsNum);
        break;
      case "%":
        oEl.textContent = `${d3.format(".1f")(fldValAsNum)}%`;
        break;
      case "breakpoint":
        oEl.textContent =
          fldValAsNum === glob.labelNeverBreakpoint
            ? "Never"
            : `Stage ${fldVal}`;
        break;
      case "font":
        oEl.textContent = fontWeights[fldValAsNum] ?? fldVal;
        break;
      case "align":
        oEl.textContent = alignLabels.get(fldValAsNum) ?? fldVal;
        break;
      default:
  ...
  function scaledPNG(scale) {
    ...
    const canvgObj = canvg.Canvg.fromString(canvasContext, svgContent, {
      ignoreMouse: true,
  ...
  function settingIsValid(sData, hVal, cfg) {
    ...
    if (
      ["whole", "contained", "breakpoint"].includes(dataType) &&
  ...
  function getHumanValueFromPage(fName, dataType) {
    switch (dataType) {
      case "radio":
        return radioRef(fName).value;
      case "color":
        return el(fName).value.toLowerCase();
      // translate true/false BACK to Y/N in this case:
      case "yn":
        return el(fName)?.checked ? "Y" : "N";
      case "list":
      case "text":
        return el(fName).value;
      // All remaining types are numeric:
      default:
        return Number(el(fName).value);
    }
  }
  ...
  glob.replaceGraphConfirmed = () => {
    ...
    Object.entries(savedRecipe.settings).forEach(([fld, newVal]) => {
      ...
      if (validSetting) {
        setValueOnPage(fld, fldData[0], finalValue);
      }
    });
  ...
  function render_sankey(allNodes, allFlows, cfg, numberStyle) {
    ...
    function setUpTextDimensions() {
      ...
      function isFirefox() {
        return (
          navigator &&
          /firefox/i.test(navigator.userAgent || navigator.vendor || "")
        );
      }
    ...
    function setUpDiagramSize() {
      ...
      const graphW = cfg.size_w - cfg.margin_l - cfg.margin_r,
        ...
        minimumW =
          stagesArr.length * cfg.node_w + lastStage * (cfg.node_border + 5),
    ...
    allNodes.filter(shadowFilter).forEach((n) => {
      ...
      if (typeof n.color === "undefined" || n.color === "") {
        ...
        const colorKeyString = (n.tipname?.match(/^\s*(\S+)/) || [
          null,
          "name-is-blank",
        ])[1];
      ...
      n.border_color = darkBg
        ? d3.rgb(n.color).brighter(2)
        : d3.rgb(n.color).darker(2);
    ...
    function applyNodeMove(index) {
      ...
      d3.selectAll(`#sankey_svg .${n.css_class}`).attr(
        "transform",
        isAZeroMove(n.move)
          ? null
      ...
      allNodes
        ...
        .forEach((n) => {
          ...
          const labelTextSelector = `#${n.label.dom_id}`,
            labelBB = diagLabels.select(labelTextSelector).node().getBBox(),
            bg = n.label.bg;
  ...
  function getDiagramDefinition(verbose) {
    ...
    function outputFldName(fld) {
      const prefixLen = currentSettingGroup.length,
        shortFldName =
          prefixLen && fld.startsWith(`${currentSettingGroup}_`)
  ...
  glob.process_sankey = () => {
    ...
    [1, 2, 4, 6].forEach((s) => {
      el(`save_as_png_${s}x`).title = `PNG image file: ${
        approvedCfg.size_w * s
      } x ${approvedCfg.size_h * s}`;
    });
    ...
    function explainSum(n, dir) {
      ...
      const breakdown = flowGroup
        .map((f) => f.value)
        .sort((a, b) => b - a)
        .map((v) => withUnits(v))
        .join(" + ");
      return (
        `<dfn title="${formattedSum} from ${flowCt} ` +
...
document.getElementById("toggleDropdown").addEventListener("click", (e) => {
  e.preventDefault(); // Prevent the default link behavior
  const dropdown = document.getElementById("example_diagrams");
...
function toggleBox(boxId) {
  ...
  // Toggle visibility of the clicked box
  if (!isVisible) {
